...
<template>
	...
	>
		<div class="flex flex-col gap-6">
			...
			<div
				v-if="props.currentProject.repositoryUrl"
			...
			<!-- Empty State -->
			<div
				v-if="!props.projectConfigs || props.projectConfigs.length === 0"
				class="empty-state"
			>
			...
			<div v-else class="flex flex-col gap-3">
				...
				<ul class="max-h-80 overflow-y-auto space-y-2 pr-2">
					<li v-for="(config, index) in props.projectConfigs" :key="index">
						<div
							class="card-item cursor-pointer group"
							@click="handleSelectServerConfig(config)"
						>
							<div class="flex items-start justify-between min-w-0">
								<div class="flex-1 space-y-3 min-w-0">
									...
									<div class="flex items-start gap-2 min-w-0">
										<Icon
											icon="mdi:identifier"
											class="w-5 h-5 text-purple-400 mt-0.5 shrink-0"
										/>
										<div class="flex-1 min-w-0">
											<p class="text-xs text-slate-400 mb-1">
								...
								<Icon
									...
									class="w-6 h-6 text-slate-500 group-hover:text-blue-400 transition-colors shrink-0 ml-2"
								/>
							</div>
						</div>
					</li>
