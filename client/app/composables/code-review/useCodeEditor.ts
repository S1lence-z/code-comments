...
import {
	showCommentFormEffect,
	hideCommentFormEffect,
} from "../../code-mirror/comment-form-extensions";
...
export const useCodeEditor = (props: CodeEditorProps, emit: CodeEditorEmits) => {
	...
	const handleEditCommentInline = (commentId: string): void => {
		...
		const endLineNumber =
			comment.type === CommentType.Multiline ? comment.location.endLineNumber : null;
	...
	const handleCommentUpsert = async (
		...
		const commentData: RawCommentData = {
			...
			content: content,
			startLineNumber:
				activeFormState.value.startLineNumber || activeFormState.value.lineNumber,
	...
	const handleReplyCommentInline = (parentCommentId: string): void => {
		...
		const endLineNumber =
			rootComment.type === CommentType.Multiline ? rootComment.location.endLineNumber : null;
